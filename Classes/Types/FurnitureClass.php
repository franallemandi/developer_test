...
class FurnitureClass extends MainClass
{
    ...
    public function sqlQuery($values)
    {
        ...
        $name = $values['name'];
        $price = $values['price'] . " $";
        $description = "Dimension: " . $values['height'] . "x" . $values['width'] . "x" . $values['length'];
    ...
    public function skuCheck($values)
    {
        ...
        for ($i = 0; $i < count($list_info); $i++) {
            ...
            if (
                $list_info[$i]->sku == $sku && ($list_info[$i]->name != $name
                || $list_info[$i]->price != $price || $list_info[$i]->description != $description)
            ) {
